...
export default function Project({title, description,tags,imageUrl,url}:ProjectProps) { 
    ...
    return <section ref={ref} className='group bg-gray-100 max-w-[42rem] border 
    border-black/5 overflow-hidden sm:pr-8 relative sm:h-[20rem] 
    mb-3 sm:mb-8 last:mb-0 sm:group-even:pl-8 dark:bg-white/10 dark:hover:bg-white/20 dark:text-white'>
        <div className='pt-4 pb-7 px-5 sm:pl-10 sm:pr-2 sm:pt-10 
        sm:max-w-[50%] flex flex-col h-full sm:group-even:ml-[18rem]'>
            <a href={url} target='_blank'><h3 className='text-2xl font-semibold'>{title}</h3></a>
            <p className='mt-2 leading-relaxed text-gray-700 dark:text-white/70'>{description}</p>
            <ul className='flex flex-wrap gap-2 mt-4 sm:mt-auto '>
                {
                    tags.map((tag,index) => (
                        <li className='bg-black/[0.7] px-3 py-1 
        ...
        <a href={url} target='_blank'><Image src={imageUrl} alt='Placeholder' quality={95} 
        className='absolute invisible sm:visible 
        ...
        group-even:group-hover:rotate-1'></Image></a>
    </section>
}
